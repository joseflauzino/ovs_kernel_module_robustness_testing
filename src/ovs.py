...
try:
    ...
    from pyroute2.netlink import NLA_F_NESTED, NLM_F_ACK, NLM_F_CREATE, NLM_F_DUMP, NLM_F_REQUEST
...
array_class = nlmsg_atoms.array

# Define the new method
def custom_encode(self):
    # Handle the case where self.value is None
    if self.value is None:
        self['value'] = b''  # Encode as an empty byte sequence
    elif type(self.value) not in [list, tuple]:
        fmt = '%s%i%s' % (self.fmt[:-1], 1, self.fmt[-1:])  # Format for one element
        self['value'] = struct.pack(fmt, self.value)
    else:
        fmt = '%s%i%s' % (self.fmt[:-1], len(self.value), self.fmt[-1:])
        self['value'] = struct.pack(fmt, *self.value)

    # Call the original encode() from the superclass
    super(array_class, self).encode()

# Apply monkey patch
array_class.encode = custom_encode

class float(nla_base):
    __slots__ = ()
    sql_type = 'FLOAT' # https://www.dofactory.com/sql/float

    fields = [('value', 'f')] # https://docs.python.org/3/library/struct.html

nlmsg_atoms.float = float
...
DEFAULT_DP_NAME = "dp-default-name"
DEFAULT_VPORT_NAME= "vport1"
...
def send_gnl_msg(gnl_family_obj, msg, flags):
    """
        Generic method to send Generic Netlink messages.
    """
    nl_error_code = None
    nl_error_msg = None
    reply_msg = None
    try:
        reply = gnl_family_obj.nlm_request(
            msg, msg_type=gnl_family_obj.prid, msg_flags=flags
        )
        reply_msg = reply[0]
    except NetlinkError as ne:
        nl_error_code = ne.code
        nl_error_msg = os.strerror(nl_error_code)
    return nl_error_code, nl_error_msg, reply_msg

def existsAttr(attr:str, attr_list:list):
...
class meterband(nla):
    nla_map = (
        ("OVS_BAND_ATTR_UNSPEC", "none"),
        ("OVS_BAND_ATTR_TYPE", "uint32"),
        ("OVS_BAND_ATTR_RATE", "uint32"),
        ("OVS_BAND_ATTR_BURST", "uint32"),
        ("OVS_BAND_ATTR_STATS", "flowstats"),
    )
    ...
    class flowstats(nla):
        ...
        fields_map = (
            ("packets", lambda x: int(x) if x else 0),
            ("bytes", lambda x: int(x) if x else 0),
        )
        

        def parse(self, flowstr):
            for field in self.fields_map:
                if flowstr.startswith(field[0]):
                    flowstr = flowstr[len(field[0]) + 1 :]
                    splitchar = 0
                    for c in flowstr:
                        if c == "," or c == ")":
                            break
                        splitchar += 1
                    data = flowstr[:splitchar]
                    flowstr = flowstr[splitchar+1:]
                else:
                    data = ""
                if field[0] == "packets":
                    self["packets"] = field[1](data)
                elif field[0] == "bytes":
                    self["bytes"] = field[1](data)
    
    def parse(self, meterstr):
        param_mapping = (
            ("OVS_BAND_ATTR_TYPE", "type", lambda x: int(x)),
            ("OVS_BAND_ATTR_RATE", "rate", lambda x: int(x)),
            ("OVS_BAND_ATTR_BURST", "burst", lambda x: int(x)),
            ("OVS_BAND_ATTR_STATS", "stats", meterband.flowstats)
        )
        remaining_str = meterstr.strip()

        while remaining_str:
            parsed = False
            for attr_name, param_name, parse_func in param_mapping:
                # Check if the remaining string starts with the parameter name
                if remaining_str.startswith(param_name + '('):
                    # Extract the content within the parentheses
                    start = len(param_name) + 1
                    paren_count = 1
                    end = start
                    while end < len(remaining_str) and paren_count > 0:
                        if remaining_str[end] == '(':  # Nested parenthesis
                            paren_count += 1
                    ...
                    value_str = remaining_str[start:end-1]
                    remaining_str = remaining_str[end:].lstrip(', ')

                    # Parse the value using the provided function
                    if not isinstance(parse_func, types.FunctionType): # it is flowstats
                        obj = parse_func()
                        obj.parse(value_str)
                        self['attrs'].append([attr_name, obj])
                    else:
                        parsed_value = parse_func(value_str)
                        self['attrs'].append([attr_name, parsed_value])
                    parsed = True
                    break
            if not parsed:
                raise ValueError(f"Unknown attribute in input string: '{remaining_str}'")
...
class zonelimit(nla):
    ...
    fields_map = (
        ("zone_id", lambda x: int(x) if x else 0),
        ("limit", lambda x: int(x) if x else 0),
        ("count", lambda x: int(x) if x else 0),
    )

    def parse(self, zonelimitstr):
        for field in self.fields_map:
            if zonelimitstr.startswith(field[0]):
                zonelimitstr = zonelimitstr[len(field[0]) + 1 :]
                splitchar = 0
                for c in zonelimitstr:
                    if c == "," or c == ")":
                        break
                    splitchar += 1
                data = zonelimitstr[:splitchar]
                zonelimitstr = zonelimitstr[splitchar+1:]
                if zonelimitstr.startswith(','):
                    zonelimitstr = zonelimitstr[1:]
            else:
                data = ""
            if data != "":
                self[field[0]] = field[1](data)
            else:
                raise ValueError(f"Invalid value for the attribute in input string: '{zonelimitstr}'")

nlmsg_atoms.ovskey = ovskey
nlmsg_atoms.ovsactions = ovsactions
nlmsg_atoms.meterband = meterband
nlmsg_atoms.zonelimit = zonelimit
...
class OvsPacket(GenericNetlinkSocket):
    ...
    class ovs_packet_msg(ovs_dp_msg):
        nla_map = (
            ("OVS_PACKET_ATTR_UNSPEC", "none"), # original: none
            ("OVS_PACKET_ATTR_PACKET", "array(uint8)"), # original: array(uint8)
            ("OVS_PACKET_ATTR_KEY", "ovskey"), # original: ovskey
            ("OVS_PACKET_ATTR_ACTIONS", "ovsactions"), # original: ovsactions
            ("OVS_PACKET_ATTR_USERDATA", "none"), # original: none
            ("OVS_PACKET_ATTR_EGRESS_TUN_KEY", "none"), # original: none
            ("OVS_PACKET_ATTR_UNUSED1", "none"), # original: none
            ("OVS_PACKET_ATTR_UNUSED2", "none"), # original: none
            ("OVS_PACKET_ATTR_PROBE", "uint8"), # original: none
            ("OVS_PACKET_ATTR_MRU", "int32"), # original: uint16
            ("OVS_PACKET_ATTR_LEN", "int64"), # original: uint32
            ("OVS_PACKET_ATTR_HASH", "float"), # original: uint64
        )
    ...
    def send_msg(self, cmd, attrs=[], reserved=0):
        msg = OvsPacket.ovs_packet_msg()
        if_index = OvsDatapath().get_dp_index()
        if if_index:
            msg["dpifindex"] = if_index
        else:
            nl_error_msg = "DP "+DEFAULT_DP_NAME+" not found."
            return None, -1, nl_error_msg, None
        ...
        if cmd == OVS_PACKET_CMD_EXECUTE:
            # Check if OVS_PACKET_ATTR_KEY exists or not
            if not existsAttr("OVS_PACKET_ATTR_KEY", attrs):
                k = ovskey()
                m = ovskey()
                flowstr="in_port(1),eth(),eth_type(0x0806),arp()"
                k.parse(flowstr, m)
                msg["attrs"].append(["OVS_PACKET_ATTR_KEY", k])

            # Check if OVS_PACKET_ATTR_ACTIONS exists
            if not existsAttr("OVS_PACKET_ATTR_ACTIONS", attrs):
                a = ovsactions()
                actstr= "2"
                a.parse(actstr)
                msg["attrs"].append(["OVS_PACKET_ATTR_ACTIONS", a])
            
        nl_error_code, nl_error_msg, reply_msg = send_gnl_msg(self, msg, flags)
        
        return msg, nl_error_code, nl_error_msg, reply_msg
...
class OvsDatapath(GenericNetlinkSocket):
    ...
    class dp_cmd_msg(ovs_dp_msg):
        """
        Message class that will be used to manage OVS Datapaths.
        """

        nla_map = (
            ("OVS_DP_ATTR_UNSPEC", "int64"), # original: none
            ("OVS_DP_ATTR_NAME", "asciiz"), # original: asciiz
            ("OVS_DP_ATTR_UPCALL_PID", "array(int64)"), # original: array(uint32)
            ("OVS_DP_ATTR_STATS", "dpstats"), # original: dpstats
            ("OVS_DP_ATTR_MEGAFLOW_STATS", "megaflowstats"), # original: megaflowstats
            ("OVS_DP_ATTR_USER_FEATURES", "int64"), # original: uint32
            ("OVS_DP_ATTR_PAD", "uint64"), # original: none
            ("OVS_DP_ATTR_MASKS_CACHE_SIZE", "int64"), # original: uint32
            ("OVS_DP_ATTR_PER_CPU_PIDS", "array(uint32)"), # original: array(uint32)
        )
    ...
    def get_dp_index(self, dp_name=DEFAULT_DP_NAME):
        msg = OvsDatapath.dp_cmd_msg()
        msg["cmd"] = OVS_DP_CMD_GET
        msg["version"] = OVS_DATAPATH_VERSION
        msg["reserved"] = 0
        msg["dpifindex"] = 0
        msg["attrs"].append(["OVS_DP_ATTR_NAME", dp_name])
        ...
        if reply is None:
            return 0
        return reply["dpifindex"]
    
    def send_msg(self, cmd, attrs=[], reserved=0, ifindex=0):
        # msg = family.dp_cmd_msg()
        msg = self.dp_cmd_msg()
        msg["cmd"] = cmd
        msg["version"] = OVS_DATAPATH_VERSION
        msg["reserved"] = reserved
        ...
        #TODO: add other mandatory parameters
        
        flags = NLM_F_REQUEST
        if cmd in [OVS_DP_CMD_NEW, OVS_DP_CMD_DEL, OVS_DP_CMD_SET]:
            flags =  NLM_F_REQUEST | NLM_F_ACK
            if not existsAttr("OVS_DP_ATTR_UPCALL_PID", attrs):
                msg["attrs"].append(["OVS_DP_ATTR_UPCALL_PID", get_upcall_pid_array()])

            if not existsAttr("OVS_DP_ATTR_USER_FEATURES", attrs):
                dpfeatures = 0
                dpfeatures |= OvsDatapath.OVS_DP_F_DISPATCH_UPCALL_PER_CPU
                dpfeatures &= ~OvsDatapath.OVS_DP_F_VPORT_PIDS
                msg["attrs"].append(["OVS_DP_ATTR_USER_FEATURES", dpfeatures])

        if not existsAttr("OVS_DP_ATTR_NAME", attrs):
            msg["attrs"].append(["OVS_DP_ATTR_NAME", DEFAULT_DP_NAME])
...
class OvsVport(GenericNetlinkSocket):
    ...
    class ovs_vport_cmd_msg(ovs_dp_msg):
        nla_map = (
            ("OVS_VPORT_ATTR_UNSPEC", "none"), # original: none
            ("OVS_VPORT_ATTR_PORT_NO", "int64"), # original: uint32
            ("OVS_VPORT_ATTR_TYPE", "int64"), # original: uint32
            ("OVS_VPORT_ATTR_NAME", "asciiz"), # original: asciiz
            ("OVS_VPORT_ATTR_OPTIONS", "vportopts"), # original: vportopts
            ("OVS_VPORT_ATTR_UPCALL_PID", "array(int64)"), # original: array(uint32)
            ("OVS_VPORT_ATTR_STATS", "vportstats"), # original: vportstats
            ("OVS_VPORT_ATTR_PAD", "none"), # original: none
            ("OVS_VPORT_ATTR_IFINDEX", "int64"), # original: uint32
            ("OVS_VPORT_ATTR_NETNSID", "int64"), # original: uint32
        )
    ...
    def __init__(self, packet=OvsPacket()):
        GenericNetlinkSocket.__init__(self)
        self.bind(OVS_VPORT_FAMILY, OvsVport.ovs_vport_cmd_msg)
        self.upcall_packet = packet
    
    def send_msg(self, cmd, attrs=[], reserved=0):
        msg = OvsVport.ovs_vport_cmd_msg()
        if_index = OvsDatapath().get_dp_index()
        if if_index:
            msg["dpifindex"] = if_index
        else:
        ...
        flags = NLM_F_REQUEST
        if cmd != OVS_VPORT_CMD_GET:
            flags =  NLM_F_REQUEST | NLM_F_ACK

        if cmd == OVS_VPORT_CMD_NEW:
            if not existsAttr("OVS_VPORT_ATTR_TYPE", attrs):
                msg["attrs"].append(["OVS_VPORT_ATTR_TYPE", OvsVport.OVS_VPORT_TYPE_NETDEV])

            if not existsAttr("OVS_VPORT_ATTR_NAME", attrs):
                msg["attrs"].append(["OVS_VPORT_ATTR_NAME", DEFAULT_VPORT_NAME])
            
            if not existsAttr("OVS_VPORT_ATTR_UPCALL_PID", attrs):
                msg["attrs"].append(["OVS_VPORT_ATTR_UPCALL_PID", get_upcall_pid_array()])
...
class OvsFlow(GenericNetlinkSocket):
    class ovs_flow_cmd_msg(ovs_dp_msg):
        nla_map = (
            ("OVS_FLOW_ATTR_UNSPEC", "none"), #original: none
            ("OVS_FLOW_ATTR_KEY", "ovskey"), #original: ovskey
            ("OVS_FLOW_ATTR_ACTIONS", "ovsactions"), #original: ovsactions
            ("OVS_FLOW_ATTR_STATS", "flowstats"), #original: flowstats
            ("OVS_FLOW_ATTR_TCP_FLAGS", "int32"), #original: uint8
            ("OVS_FLOW_ATTR_USED", "int64"), #original: uint64
            ("OVS_FLOW_ATTR_CLEAR", "int64"), #original: none
            ("OVS_FLOW_ATTR_MASK", "ovskey"), #original: ovskey
            ("OVS_FLOW_ATTR_PROBE", "int16"), #original: none
            ("OVS_FLOW_ATTR_UFID", "array(int64)"), #original: array(uint32)
            ("OVS_FLOW_ATTR_UFID_FLAGS", "int64"), #original: uint32
        )
    ...
    def __init__(self):
        ...
        self.bind(OVS_FLOW_FAMILY, OvsFlow.ovs_flow_cmd_msg)

    def send_msg(self, cmd, attrs=[], reserved=0):  
        msg = OvsFlow.ovs_flow_cmd_msg()
        if_index = OvsDatapath().get_dp_index()
        if if_index:
            msg["dpifindex"] = if_index
        else:
        ...
        # 6bb38b00-00b9-4c7f-a7b4-b1a1b57d1fd1
        ufid_array = (52156980842957568, -5369100195433500255)

        if cmd == OVS_FLOW_CMD_NEW:
            flowstr="in_port(2),eth(),eth_type(0x0806),arp()"
            actstr= "1"
            # a26935ce-902d-43f2-bccd-34eac6f51bf7
            ufid_array = (-8057709448909998642, -4110348334067272470)

        # Check if OVS_FLOW_ATTR_KEY exists or not
        if not existsAttr("OVS_FLOW_ATTR_KEY", attrs):
            k = ovskey()
            m = ovskey()
            
            k.parse(flowstr, m)
            msg["attrs"].append(["OVS_FLOW_ATTR_KEY", k])

        if not existsAttr("OVS_FLOW_ATTR_UFID", attrs):
            msg["attrs"].append(["OVS_FLOW_ATTR_UFID", ufid_array])

        # Check if OVS_FLOW_ATTR_ACTIONS exists when executing command OVS_FLOW_CMD_NEW
        if cmd == OVS_FLOW_CMD_NEW and not existsAttr("OVS_FLOW_ATTR_ACTIONS", attrs):
            a = ovsactions()
            a.parse(actstr)
            msg["attrs"].append(["OVS_FLOW_ATTR_ACTIONS", a])
        
        # Add flags properly
        flags = NLM_F_REQUEST
        if cmd != OVS_FLOW_CMD_GET:
            flags |=  NLM_F_ACK
...
class OvsMeter(GenericNetlinkSocket):
    class ovs_meter_cmd_msg(ovs_dp_msg):
        nla_map = (
            ("OVS_METER_ATTR_UNSPEC", "none"), #original: none
            ("OVS_METER_ATTR_ID", "int64"), #original: uint
            ("OVS_METER_ATTR_KBPS", "int64"), #original: none
            ("OVS_METER_ATTR_STATS", "flowstats"), #original: flowstats
            ("OVS_METER_ATTR_BANDS", "meterband"), #original: meterband
            ("OVS_METER_ATTR_USED", "float"), #original: uint64
            ("OVS_METER_ATTR_CLEAR", "int64"), #original: none
            ("OVS_METER_ATTR_MAX_METERS", "int64"), #original: uint32
            ("OVS_METER_ATTR_MAX_BANDS", "int64"), #original: uint32
            ("OVS_METER_ATTR_PAD", "int64"), #original: none
        )
    ...
    def __init__(self):
        ...
        self.bind(OVS_METER_FAMILY, OvsMeter.ovs_meter_cmd_msg)

    def send_msg(self, cmd, attrs=[], reserved=0):  
        msg = OvsMeter.ovs_meter_cmd_msg()
        if_index = OvsDatapath().get_dp_index()
        if if_index:
            msg["dpifindex"] = if_index
        else:
        ...
        # Check if OVS_METER_ATTR_ID exists or not
        mandatory_for_id = [OVS_METER_CMD_SET, OVS_METER_CMD_GET, OVS_METER_CMD_DEL]
        if cmd in mandatory_for_id and not existsAttr("OVS_METER_ATTR_ID", attrs):
            msg["attrs"].append(["OVS_METER_ATTR_ID", 1])

        # Check if OVS_METER_ATTR_BANDS exists when executing command OVS_METER_CMD_SET
        if cmd == OVS_METER_CMD_SET and not existsAttr("OVS_METER_ATTR_BANDS", attrs):
            meterstr = "type(100),rate(1265),burst(200),stats(packets=50,bytes=2546)"
            bands = meterband()
            bands.parse(meterstr)
            msg["attrs"].append(["OVS_METER_ATTR_BANDS", [bands]])
        
        # Add flags properly
        flags = NLM_F_REQUEST
        if cmd in [OVS_METER_CMD_SET]:
            flags =  NLM_F_REQUEST | NLM_F_ACK
...
class OvsCtLimit(GenericNetlinkSocket):
    class ovs_ct_limit_cmd_msg(ovs_dp_msg):
        nla_map = (
            ("OVS_CT_LIMIT_ATTR_ZONE_LIMIT", "zonelimit"),
        )

    def __init__(self):
        GenericNetlinkSocket.__init__(self)

        self.bind(OVS_CT_LIMIT_FAMILY, OvsCtLimit.ovs_ct_limit_cmd_msg)

    def send_msg(self, cmd, attrs=[], reserved=0):  
        msg = OvsCtLimit.ovs_ct_limit_cmd_msg()
        if_index = OvsDatapath().get_dp_index()
        if if_index:
            msg["dpifindex"] = if_index
        else:
        ...
        # Check if OVS_CT_LIMIT_ATTR_ZONE_LIMIT exists or not
        mandatory = [OVS_CT_LIMIT_CMD_SET, OVS_CT_LIMIT_CMD_DEL]
        if cmd in mandatory and not existsAttr("OVS_CT_LIMIT_ATTR_ZONE_LIMIT", attrs):
            zone_limit = zonelimit()
            zone_limit.parse("zone_id(1),limit(2),count(10)")
            msg["attrs"].append(["OVS_CT_LIMIT_ATTR_ZONE_LIMIT", zone_limit])
        
        # Add flags properly
        flags = NLM_F_REQUEST
        if cmd in [OVS_CT_LIMIT_CMD_SET]:
            flags =  NLM_F_REQUEST | NLM_F_ACK
